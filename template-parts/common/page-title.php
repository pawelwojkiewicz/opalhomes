...
if (!opalhomes_page_enable_breadcrumb()) {
    return;
}
...
$otf_home = esc_html__('Home', 'opalhomes');
$otf_blog = esc_html__('Blog', 'opalhomes');
$otf_shop = esc_html__('Shop', 'opalhomes');
$otf_title = '';
...
if (!function_exists('bcn_display')) {
    ...
    if (!is_front_page()) {
        ...
        if (is_category()) {
            ...
            $otf_title = esc_html__('Day', 'opalhomes');
            // Day archive

            // Year link
            $otf_output .= '<li class="list-inline-item item"><a href="' . esc_url(get_year_link(get_the_time('Y'))) . '" title="' . esc_attr(get_the_time('Y')) . '">' . get_the_time('Y') . esc_html__(' Archives', 'opalhomes') . '</a></li>';
            $otf_output .= '<li class="list-inline-item separator"> ' . wp_kses_post($otf_sep) . ' </li>';

            // Month link
            $otf_output .= '<li class="list-inline-item item"><a href="' . esc_url(get_month_link(get_the_time('Y'), get_the_time('m'))) . '" title="' . esc_attr(get_the_time('M')) . '">' . get_the_time('M') . esc_html__(' Archives', 'opalhomes') . '</a></li>';
            $otf_output .= '<li class="list-inline-item separator"> ' . wp_kses_post($otf_sep) . ' </li>';

            // Day display
            $otf_output .= '<li class="list-inline-item item current"> ' . get_the_time('jS') . ' ' . get_the_time('M') . esc_html__(' Archives', 'opalhomes') . '</li>';

        } elseif (is_month()) {
            $otf_title = get_the_time('M') . esc_html__(' Archives', 'opalhomes');
            // Month Archive
            ...
            $otf_title = get_the_time('Y') . esc_html__('Archives', 'opalhomes');
            ...
            if (!empty($author->ID)) {
                ...
                $otf_title = esc_html__('Author: ', 'opalhomes') . $otf_userdata->display_name;
                // Get the author information
                $otf_output .= '<li class="list-inline-item item current">' . esc_html__('Author: ', 'opalhomes') . '<a href="' . get_author_posts_url($otf_userdata->ID, $otf_userdata->nice_name) . '">' . esc_html($otf_userdata->display_name) . '</a></li>';
            }

        } elseif (get_query_var('paged')) {
            // Paginated archives
            $otf_output .= '<li class="list-inline-item item current">' . esc_html__('Page', 'opalhomes') . ' ' . get_query_var('paged', '1') . '</li>';

        } elseif (is_search()) {
            $otf_title = esc_html__('Search', 'opalhomes');
            $otf_output .= '<li class="list-inline-item item current">' . esc_html__('Keyword: ', 'opalhomes') . get_search_query() . '</li>';

        } elseif (is_404()) {
            $otf_title = esc_html__('Error 404', 'opalhomes');
            $otf_output .= '<li class="list-inline-item item current">' . wp_kses_post($otf_title) . '</li>';
        }
    }
    $otf_output .= '</ul>';
...
(!is_front_page()) {
    if (is_home()) {
        ...
        $otf_title = get_the_time('D') . esc_html__(' Archives', 'opalhomes');
    } elseif (is_month()) {
        $otf_title = get_the_time('M') . esc_html__(' Archives', 'opalhomes');
    } elseif (is_year()) {
        $otf_title = get_the_time('Y') . esc_html__('Archives', 'opalhomes');
        ...
        global $author;
        if (!empty($author->ID)) {
            $otf_title = esc_html__('Author', 'opalhomes');
        }
    ...
    <div class="wrap w-100 d-flex align-items-center justify-content-center text-center">
        <div class="page-title-bar-inner d-flex flex-column">
            <?php if (is_page() || is_single()) { ?>
                ...
                <?php }
            } elseif (is_404() || is_archive() || is_search() || is_home()) { ?>
                <div class="page-header">
